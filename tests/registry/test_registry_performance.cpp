...
TEST_CASE("Registry | Bulk insertion", "[Registry][Benchmark][Insert]")
{
    ...
    BENCHMARK_FUNCTION("Registry | Insert 10k entities and shapes",
                       15ms,
                       (
                           [&]()
                           {
    ...
    BENCHMARK_FUNCTION("Registry | Insert 100k entities and shapes",
                       150ms,
                       (
                           [&]()
                           {
...
TEST_CASE("Registry | Collision query dynamic", "[Registry][Query][AllPairs][Benchmark]")
{
    ...
    BENCHMARK_FUNCTION("Registry | Find all colliding pairs among 1k entities",
                       30us,
    ...
    BENCHMARK_FUNCTION("Registry | Find all colliding pairs among 10k entities",
                       3ms,
                       [&]() { return reg.getCollidingPairs().size(); });
...
TEST_CASE("Registry | Collision query bullets", "[Registry][Query][AllPairs][Benchmark]")
{
    ...
    BENCHMARK_FUNCTION(
        "Registry | Find all colliding pairs among 1k bullets", 30us, [&]() { return reg.getCollidingPairs().size(); });
    ...
    BENCHMARK_FUNCTION(
        "Registry | Find all colliding pairs among 10k bullets", 5ms, [&]() { return reg.getCollidingPairs().size(); });
