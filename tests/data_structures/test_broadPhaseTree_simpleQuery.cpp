...
TEST_CASE("BroadPhaseTree | query finds all overlapping proxies", "[BroadPhaseTree][Query]")
{
    ...
    for (uint32_t i = 1; i < 5; i++)
        for (uint32_t j = 1; j < 5; j++)
        {
            ...
            expectedOverlaps.emplace_back(Vec2{x, y}, Vec2{x + 1.0f, y + 1.0f});
        }
    ...
    for (uint32_t i = 0; i < gridSize; ++i)
    {
        for (uint32_t j = 0; j < gridSize; ++j)
        {
            ...
            const bool isExpectedOverlap =
                std::find(expectedOverlaps.begin(), expectedOverlaps.end(), aabb) != expectedOverlaps.end();
...
TEST_CASE("BroadPhaseTree | firstHitRaycast returns id, entry, and exit for closest hit",
          "[BroadPhaseTree][FirstHitRaycast]")
{
    BroadPhaseTree<uint32_t> tree;
...
TEST_CASE("BroadPhaseTree | piercingRaycast with infinite ray finds intersected proxies",
          "[BroadPhaseTree][PiercingRaycast]")
{
    BroadPhaseTree<uint32_t> tree;
...
TEST_CASE("BroadPhaseTree | firstHitRaycast with infinite ray finds the nearest hit",
          "[BroadPhaseTree][FirstHitRaycast]")
{
    BroadPhaseTree<uint32_t> tree;
...
TEST_CASE("BroadPhaseTree | piercingRaycast with infinite ray finds all hits with entry/exit points",
          "[BroadPhaseTree][PiercingRaycast]")
{
    BroadPhaseTree<uint32_t> tree;
...
TEST_CASE("BroadPhaseTree | firstHitRaycast with infinite ray returns id, entry, and exit for closest hit",
          "[BroadPhaseTree][FirstHitRaycast]")
{
    BroadPhaseTree<uint32_t> tree;
...
TEST_CASE("BroadPhaseTree batchQuery with multiple threads", "[BroadPhaseTree][BatchQuery]")
{
    ...
    {
        queries.emplace_back(AABB{Vec2{float(i * 10), float(i * 10)}, Vec2{float((i + 1) * 10), float((i + 1) * 10)}},
                             ~0ull);
