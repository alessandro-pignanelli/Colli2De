...
TEST_CASE("DynamicBVH | piercingRaycast with infinite ray finds all hits with entry/exit points",
          "[DynamicBVH][PiercingRaycast]")
{
    const float margin = 0.0f;
    DynamicBVH<uint32_t> bvh(margin);
...
TEST_CASE("DynamicBVH | firstHitRaycast with infinite ray returns id, entry, and exit for closest hit",
          "[DynamicBVH][FirstHitRaycast]")
{
    const float margin = 0.0f;
    DynamicBVH<uint32_t> bvh(margin);
...
TEST_CASE("DynamicBVH | findAllCollisions finds correct pairs", "[DynamicBVH][BroadPhaseCollisions]")
{
    ...
    bvh.findAllCollisions([&](uint32_t id1, uint32_t id2) { pairs.emplace_back(id1, id2); });
